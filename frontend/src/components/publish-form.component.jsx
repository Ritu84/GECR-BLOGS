...
const PublishForm = () => {
  ...
  const handleKeyDown=(e)=>{
    if (e.keyCode === 13 || e.keyCode === 188){
           //user has pressed enter or comma key
           e.preventDefault();
  ...
  const publishBlog=(e)=>{
    ...
    if(!des.length || des.length> characterLimit){
      return toast.error("Description must be between 1-"+ characterLimit +" characters");
    }
    if(!tags.length){
      return  toast.error("Atleast one Tag is needed")
    }
  ...
  .catch(({ response})=>{
    ...
    return (
        <AnimationWrapper>
          <section className="w-screen min-h-screen grid  items-center lg:grid-cols-2 py-16 lg:gap-4 md:gap-0 md:grid-cols-1">
            <Toaster/>
            ...
            <div className="max-w-[550px] center  ">
                ...
                <div className="w-full aspect-video  rounded-lg  overflow-hidden bg-grey mt-4 lg:w-1/2 md:w-full">
